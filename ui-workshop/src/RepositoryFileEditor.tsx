...
import {getRepositoryDocumentSession, RepositoryDocumentSessionError, type RepositoryDocumentSession} from './repository-document-session';
import {usePluginHost, useProEditorEnabled, PluginDocumentTools} from './NativePluginHost';
import {createProVisualExtension} from './pro-editor/visual';
import {ProEditorControls} from './pro-editor/ProEditorControls';
import {useDocumentSplitScroll} from './useDocumentSplitScroll';
import {ArtifactViewer} from './ArtifactViewer';
import {ReadingHistory, preserveBeforeNavigation, type ReadingLocation, type ReadingHistoryAdapter, type ReadingHistoryState, type ReadingHistoryTicket} from './reading-navigation.mjs';
import {ReadingEvidenceContext, type ReadingEvidence} from './ReadingEvidenceContext';
import {ReadingReferenceDialog} from './ReadingReferenceDialog';
import {assertResolvedRepository} from './reading-reference.mjs';
import {PluginOperationError, type PluginErrorCode, type PluginHostAdapter} from './plugin-foundation/contracts';
...
import type {RepositoryUpdateApplied, ResolvedReadingReference} from './native-types';
...
const icons: Record<string, string> = {back:'m9 3-5 5 5 5M4 8h11',forward:'m7 3 5 5-5 5M1 8h11',copy:'M6 6h8v9H6ZM10 3V1H1v10h2',pencil:'m3 10 7-7 3 3-7 7-4 1ZM9 4l3 3',search:'M11 6a5 5 0 1 1-10 0 5 5 0 0 1 10 0m-1 4 5 5',file:'M4 1h7l4 4v10H4ZM11 1v4h4',folder:'M1 3h5l2 2h7v9H1Z',panel:'M1 2h14v12H1Zm5 0v12',chevron:'m6 4 4 4-4 4'};
...
export function RepositoryFileEditor(props: Props) {
 ...
 const proEnabled = useProEditorEnabled();
 const localHistory=useRef(new ReadingHistory());
 const readingHistory=useMemo<ReadingHistoryAdapter>(()=>props.readingHistory??{state:async()=>localHistory.current.state(),checkpoint:async value=>localHistory.current.checkpoint(value),visit:async value=>localHistory.current.visit(value),peek:async direction=>localHistory.current.peek(direction),complete:async(token,value)=>localHistory.current.complete(token,value),cancel:async token=>localHistory.current.cancel(token)},[props.readingHistory]);
 const [historyState,setHistoryState]=useState<ReadingHistoryState>({back:false,forward:false,count:0,index:-1}),[historyMoving,setHistoryMoving]=useState(false);
 const historyBusy=useRef(false),historyRestore=useRef<ReadingLocation|null>(null),historyReady=useRef(false);
 const [evidence,setEvidence]=useState<ReadingEvidence|null>(null),[referenceOpen,setReferenceOpen]=useState(false);
 const [visual,setVisual]=useState(false),[split,setSplit]=useState(false);
 const reading=useRef<HTMLDivElement>(null);
 const proExtension=useMemo(()=>createProVisualExtension(),[]);
 const pluginHost=usePluginHost(),pluginHostRef=useRef(pluginHost);pluginHostRef.current=pluginHost;
 ...
 const documentSession=useRef<RepositoryDocumentSession|null>(null);
 ...
 const read=(next:string,signal?:AbortSignal):Promise<Snapshot>=>props.readSnapshot?props.readSnapshot(next,signal):readRepository({repo:repository,path:next,ref:revision},signal);
 const currentDirectory=()=>directoryRef.current?.path??active.current.path.split('/').slice(0,-1).join('/');
 const captureReadingRef=useRef<()=>ReadingLocation>(()=>{throw new Error('Reading view unavailable.');});
 const captureReadingLocation=():ReadingLocation=>({schemaVersion:1,repoId:props.repositoryIdentity??repository,repo:repository,ref:revision,path:directoryRef.current?.path??active.current.path,type:directoryRef.current?'directory':'file',mode:directoryRef.current||preview?'preview':split?'split':visual?'visual':'source',editing,visual,fragment,selection:{anchor:view.current?.state.selection.main.anchor??0,head:view.current?.state.selection.main.head??0},scroll:{main:root.current?.querySelector<HTMLElement>('.rfe-main-scroll')?.scrollTop??0,source:view.current?.scrollDOM.scrollTop??0,preview:reading.current?.scrollTop??0}});
 captureReadingRef.current=captureReadingLocation;
 const updateOptionalHistory=async(preserve:()=>Promise<unknown>,checkpoint:()=>Promise<ReadingHistoryState|null>)=>{const result=await preserveBeforeNavigation(preserve,checkpoint);if(!mounted.current)return;if(result.historyState)setHistoryState(result.historyState);else if(result.historyUnavailable)setHistoryState(previous=>({...previous,back:false,forward:false}));};
 const recordReadingLocation=async(value:ReadingLocation)=>{if(!active.current.isNew||directoryRef.current)await updateOptionalHistory(async()=>{},()=>readingHistory.visit(value));};
 useEffect(()=>{if(!ready||loading||historyReady.current)return;historyReady.current=true;const ticket=props.historyReturn;if(ticket){const restore=ticket.location;if(restore.repoId!==(props.repositoryIdentity??repository)||restore.ref!==revision||restore.path!==(directoryRef.current?.path??active.current.path)){void readingHistory.cancel(ticket.token);setError('The reading location could not be restored.');return;}setEditing(restore.editing&&local&&!active.current.readOnly);setVisual(proEnabled&&(restore.mode==='visual'||restore.visual===true));setSplit(proEnabled&&restore.mode==='split');setPreview(restore.mode==='preview');setFragment(restore.fragment??'');historyRestore.current=restore;void readingHistory.complete(ticket.token,restore).then(setHistoryState).catch(reason=>setError(reason.message));}else if(props.referenceReturn){const reference=props.referenceReturn;void restoreResolvedReference(reference).then(()=>recordReadingLocation(captureReadingLocation())).catch(reason=>setError(reason.message));}else void recordReadingLocation(captureReadingLocation()).catch(reason=>setError(reason.message));},[ready,loading]);
 useEffect(()=>{let live=true;setEvidence(null);if(!ready||directoryView||!props.loadEvidenceContext)return;void props.loadEvidenceContext({repo:repository,path,ref:revision}).then(value=>{if(live)setEvidence(value);}).catch(()=>{if(live)setEvidence({status:'unavailable',message:'Evidence context is unavailable. The document remains readable.',evidence:null});});return()=>{live=false;};},[props.loadEvidenceContext,ready,repository,revision,path,directoryView]);
 useLayoutEffect(()=>{const restore=historyRestore.current;if(!restore||loading||restore.path!==(directoryRef.current?.path??path))return;let second=0;const firstFrame=requestAnimationFrame(()=>{second=requestAnimationFrame(()=>{if(!mounted.current||historyRestore.current!==restore||restore.path!==(directoryRef.current?.path??active.current.path))return;historyRestore.current=null;const editor=view.current;if(editor&&!directoryRef.current){const length=editor.state.doc.length;editor.dispatch({selection:{anchor:Math.min(restore.selection.anchor,length),head:Math.min(restore.selection.head,length)}});editor.scrollDOM.scrollTop=restore.scroll.source;}const main=root.current?.querySelector<HTMLElement>('.rfe-main-scroll');if(main)main.scrollTop=restore.scroll.main;if(reading.current)reading.current.scrollTop=restore.scroll.preview;});});return()=>{cancelAnimationFrame(firstFrame);if(second)cancelAnimationFrame(second);};},[loading,path,directoryView,preview,editing,split,visual]);

 const showDirectory=(next:string,snapshot:Snapshot)=>{documentSession.current?.cancelOperations();const value={path:next,snapshot};directoryRef.current=value;pluginHostRef.current?.documentChanged();setDirectoryView(value);setEditing(false);clearCopyFeedback();setDirectories(previous=>({...previous,[next]:snapshot.entries}));setExpanded(previous=>{const result=new Set(previous),parts=next.split('/');for(let index=1;index<=parts.length;index++)result.add(parts.slice(0,index).join('/'));return result;});if(narrowRef.current)setCollapsed(true);};
 const flush=()=>{
 ...
 const sessionFor=(item:FileSession)=>getRepositoryDocumentSession(item,{repositoryId:repository,revision},()=>[lineNumbers(),drawSelection(),search({top:true}),syntaxHighlighting(sourceHighlightStyle),keymap.of([{key:'Mod-s',run:()=>{saveAction.current();return true;}},...searchKeymap,...defaultKeymap,...historyKeymap,indentWithTab]),settings.current.of(configuration()),EditorView.contentAttributes.of({'aria-label':'File source',spellcheck:'false'})]);
 const attachSession=(item:FileSession,editor:EditorView)=>{const next=sessionFor(item),replaced=documentSession.current!==next;if(replaced){documentSession.current?.detach(editor);documentSession.current=next;}next.attach(editor,{onChange(){refresh();checkpoint(item);},onError:setError});if(replaced)pluginHostRef.current?.documentChanged();};
 const activate=(item:FileSession,edit=editing)=>{if(view.current&&(view.current.composing||view.current.compositionStarted))throw new Error('Finish text composition before switching documents.');directoryRef.current=null;setDirectoryView(null);active.current=item;if(view.current){attachSession(item,view.current);view.current.dispatch({effects:settings.current.reconfigure(configuration())});}setSplit(false);setEditing(edit&&local&&!item.readOnly&&!readOnlyMedia(item));setPreview(!edit&&isMarkdownFile(item.proposedPath));clearCopyFeedback();setError('');refresh();if(narrowRef.current)setCollapsed(true);};
 ...
 useLayoutEffect(()=>{if(!mount.current)return;documentSession.current=sessionFor(active.current);const editor=new EditorView({parent:mount.current,state:documentSession.current.state,dispatchTransactions(transactions,current){documentSession.current?.dispatch(transactions,current);}});view.current=editor;attachSession(active.current,editor);return()=>{cancelOutlineSelection();documentSession.current?.detach(editor);view.current=null;editor.destroy();};},[]);
 ...
 useEffect(()=>{props.registerBeforeLeave?.((reason)=>{if(reason==='close')assertFilenameIntentRetained(sessions.current.values());if(busyRef.current||applyBusyRef.current)throw new Error('Wait for the current local operation before leaving.');if(view.current?.compositionStarted)throw new Error('Finish text composition before leaving.');return updateOptionalHistory(flushAction.current,()=>historyReady.current&&!active.current.isNew?readingHistory.checkpoint(captureReadingRef.current()):Promise.resolve(null));});},[props.registerBeforeLeave,readingHistory]);
 ...
 const openPath=async(next:string,kind?:'file'|'directory',nextFragment='',restore?:ReadingLocation):Promise<boolean>=>{
  if(managementHeldRef.current){setError('Recover the local file operation before navigating.');return false;}
  if(!ready||busyRef.current||applyBusyRef.current||applyHeldRef.current||confirmDiscard!==null||commitForm!==null||folderName!==null)return false;
  if(!restore&&kind==='file'&&next===active.current.path&&!active.current.isNew&&!directoryRef.current)return true;
  if(view.current?.composing){setError('Finish text composition before navigating.');return false;}
  pluginHostRef.current?.documentChanged();documentSession.current?.cancelOperations();
  request.current?.abort();const controller=new AbortController();request.current=controller;setLoading(true);setError('');
  try{
   await updateOptionalHistory(flush,()=>historyReady.current&&!active.current.isNew&&!restore?readingHistory.checkpoint(captureReadingLocation()):Promise.resolve(null));if(controller.signal.aborted)return false;
   const metadata=await read(next,controller.signal);if(controller.signal.aborted)return false;
   if(metadata?.type==='directory'){
    setFragment(nextFragment);showDirectory(next,metadata);
    if(!local){const parts=next.split('/');for(let index=1;index<parts.length;index++){const parent=parts.slice(0,index).join('/');if(directories[parent])continue;const data=await read(parent,controller.signal);if(controller.signal.aborted)return false;setDirectories(previous=>({...previous,[parent]:data.entries}));}}
    if(restore){historyRestore.current=restore;}else await recordReadingLocation({...captureReadingLocation(),path:next,type:'directory',mode:'preview',editing:false,fragment:nextFragment,scroll:{main:0,source:0,preview:0}});
    return true;
   }
   ...
   if(!item){
    ...
    if(controller.signal.aborted)return false;sessions.current.set(item.id,item);
   }
   if(controller.signal.aborted)return false;setFragment(nextFragment);activate(item,restore?restore.editing:nextFragment?false:editing);setCommit(metadata?.commit??null);
   if(restore){setVisual(proEnabled&&(restore.mode==='visual'||restore.visual===true));setSplit(proEnabled&&restore.mode==='split');setPreview(restore.mode==='preview');historyRestore.current=restore;}else await recordReadingLocation({...captureReadingLocation(),path:next,type:'file',editing:nextFragment?false:editing,mode:nextFragment||!editing&&isMarkdownFile(item.proposedPath)?'preview':'source',fragment:nextFragment,selection:{anchor:sessionFor(item).state.selection.main.anchor,head:sessionFor(item).state.selection.main.head},scroll:{main:0,source:0,preview:0}});
   ...
   return true;
  }catch(reason){if(!controller.signal.aborted)setError((reason as Error).message);return false;}finally{if(!controller.signal.aborted)setLoading(false);}
 };
 const navigateHistory=async(direction:-1|1)=>{if(historyBusy.current||busyRef.current||loading||applyBusyRef.current||applyHeldRef.current||managementHeldRef.current||view.current?.composing||document.querySelector('dialog[open]'))return;historyBusy.current=true;setHistoryMoving(true);let token:string|undefined;try{await updateOptionalHistory(flush,()=>!active.current.isNew?readingHistory.checkpoint(captureReadingLocation()):Promise.resolve(null));const ticket=await readingHistory.peek(direction);if(!ticket)return;token=ticket.token;if(ticket.location.repoId!==(props.repositoryIdentity??repository)||ticket.location.ref!==revision){if(props.onHistoryNavigate){await flush();await props.onHistoryNavigate(ticket);return;}throw new Error('This reading location belongs to a different repository or revision. Open it from the repository selector.');}const success=await openPath(ticket.location.path,ticket.location.type,ticket.location.fragment??'',ticket.location);if(success)setHistoryState(await readingHistory.complete(ticket.token,ticket.location));else setHistoryState(await readingHistory.cancel(ticket.token));}catch(reason){if(token)await readingHistory.cancel(token).catch(()=>{});if(mounted.current)setError((reason as Error).message);}finally{historyBusy.current=false;if(mounted.current)setHistoryMoving(false);}};
 const restoreResolvedReference=async(value:ResolvedReadingReference)=>{assertResolvedRepository(value,props.repositoryIdentity);if(value.status!=='resolved'||value.path!==active.current.path||value.repo!==repository||value.revision!==revision||!value.sourceHash)throw new Error('The reference does not identify this open document.');if(hasFileChanges(active.current))throw new Error('This document has a retained draft. Save or discard it before opening a pinned saved reference.');const owningSession=active.current,owningDocument=view.current?.state.doc;const bytes=new TextEncoder().encode(owningSession.buffer.getRawText()),digest=Array.from(new Uint8Array(await crypto.subtle.digest('SHA-256',bytes)),byte=>byte.toString(16).padStart(2,'0')).join('');if(!mounted.current||active.current!==owningSession||view.current?.state.doc!==owningDocument||hasFileChanges(owningSession))throw new Error('This reading request changed while resolving. Open the reference again.');if(digest!==value.sourceHash)throw new Error('These saved bytes changed after resolving the reference. Resolve it again.');if(value.target){const target=value.target,editor=view.current;if(!editor||target.from<0||target.to>editor.state.doc.length)throw new Error('The exact target is unavailable.');setPreview(false);setSplit(false);requestAnimationFrame(()=>{if(active.current!==owningSession||!view.current||view.current.state.doc!==owningDocument)return;view.current.dispatch({selection:{anchor:target.from,head:target.to},effects:EditorView.scrollIntoView(target.from,{y:'center'})});});}setNotice('');};
 const openReadingReference=async(value:ResolvedReadingReference)=>{if(value.status!=='resolved'||!value.path||!value.repo||value.revision===undefined)throw new Error('This reference is unavailable.');if(value.repo!==repository||value.revision!==revision){if(!props.onResolvedReference)throw new Error('Open this repository and revision before following this reference.');await flush();await props.onResolvedReference(value);return;}assertResolvedRepository(value,props.repositoryIdentity);const retained=findPath(value.path);if(retained&&hasFileChanges(retained))throw new Error('This document has a retained draft. Save or discard it before opening a pinned saved reference.');const success=await openPath(value.path,'file');if(!success)throw new Error('This document could not be opened.');await restoreResolvedReference(value);};
 const copyReadingReference=async()=>{const bridge=getNativeBridge();if(!bridge?.getReadingReference)return;if(hasFileChanges(active.current)){setError('Save this draft before copying a reference to saved content.');return;}try{const value=await bridge.getReadingReference({repo:repository,path:active.current.path,ref:revision});if(value.status!=='ready'||!value.reference)throw new Error(value.message);await copyText(JSON.stringify(value.reference,null,2),'Reference');}catch(reason){setError((reason as Error).message);}};
 ...
 useEffect(()=>{const target=props.navigationRequest;if(!target||target.repo!==repository||(target.ref??'')!==revision||target.id===navigationHandled.current||!ready||busy||applyBusy||applyHeld||managementHeld||loading)return;
  ...
  void(async()=>{const success=await openPath(target.path,target.type);if(!success||!mounted.current||request.current?.signal.aborted)return;
 ...
 const copyText=async(text:string,label:'Path'|'Raw file'|'Reference')=>{
 ...
 const newDrafts=[...sessions.current.values()].filter(item=>item.isNew),canEdit=!managementHeld&&!applyHeld&&!applyBusy&&!directoryView&&!media&&local&&ready&&!active.current.readOnly&&!busy&&!loading,baseModes=fileModes(active.current.proposedPath,editing),splitActive=proEnabled&&editing&&split&&isMarkdownFile(active.current.proposedPath)&&!directoryView,
  modes: ('source'|'preview'|'split')[]=media?['preview']:proEnabled&&editing&&isMarkdownFile(active.current.proposedPath)?['source','split','preview']:baseModes,
  actualPreview=!splitActive&&!directoryView&&(media||preview)&&modes.includes('preview'),selectedMode=splitActive?'split':actualPreview?'preview':'source';
 const changeMode=(mode:'source'|'preview'|'split')=>{if(view.current?.composing||view.current?.compositionStarted)return;setSplit(mode==='split');setPreview(mode==='preview');};
 useDocumentSplitScroll({enabled:splitActive,editor:view,reading,contentKey:source,resetKey:active.current.id});
 useLayoutEffect(()=>{
  const session=documentSession.current,editor=view.current,element=root.current;
  if(!session||!editor||!element)return;
  let live=true,frame:number|null=null,applied:boolean|undefined;
  const current=()=>live&&mounted.current&&documentSession.current===session&&view.current===editor&&root.current===element&&element.isConnected;
  const apply=()=>{
   if(!current()||session.isComposing)return;
   const show=proEnabled&&visual&&editing&&!splitActive&&!actualPreview&&!directoryRef.current&&!element.closest('[hidden],[inert]')&&isMarkdownFile(active.current.proposedPath);
   if(show!==applied){session.setContribution(show?proExtension:[]);applied=show;}
   editor.requestMeasure();
  };
  apply();
  const afterComposition=()=>{if(frame!==null)cancelAnimationFrame(frame);frame=requestAnimationFrame(()=>{frame=null;apply();});};
  editor.contentDOM.addEventListener('compositionend',afterComposition);
  // Repository and plugin views can hide an ancestor several levels above
  // this editor. Observe only this ancestry, not unrelated child controls.
  const visibility=new MutationObserver(apply);
  for(let ancestor:HTMLElement|null=element;ancestor;ancestor=ancestor.parentElement)visibility.observe(ancestor,{attributes:true,attributeFilter:['hidden','inert']});
  return()=>{live=false;if(frame!==null)cancelAnimationFrame(frame);visibility.disconnect();editor.contentDOM.removeEventListener('compositionend',afterComposition);};
 },[proEnabled,visual,editing,splitActive,actualPreview,proExtension,path,ready,directoryView,active.current.id]);
 const rendered=useMemo(()=>(actualPreview||splitActive)&&!media?renderSourcePreview(source,active.current.proposedPath,{externalLinks:true,technical:true,sourceMap:splitActive}):null,[actualPreview,splitActive,media,source,active.current.proposedPath]);
 ...
 const renderedDirectoryReadme=useMemo(()=>typeof directoryReadme==='string'&&directoryReadmePath?renderSourcePreview(directoryReadme,directoryReadmePath,{externalLinks:true,technical:true}):null,[directoryReadme,directoryReadmePath]);
 ...
 const afterManagement=async(result:ManagementResult)=>{
  ...
  if(result.operation==='move'){remapSessions(sessions.current.values(),result.pathMoves);pluginHostRef.current?.documentChanged();documentSession.current?.cancelOperations();}
 ...
 const pluginDocumentState=useRef({ready,loading,editable:editing&&!actualPreview,media,dialog:false});
 pluginDocumentState.current={ready,loading,editable:editing&&!actualPreview,media,dialog:confirmDiscard!==null||commitForm!==null||folderName!==null||Boolean(management.dialog)||renamingRepository||renamingEntry!==null||checkingGitHub};
 useLayoutEffect(()=>{
  if(!pluginHost)return;
  const available=()=>{const current=pluginDocumentState.current;return mounted.current&&current.ready&&!current.loading&&!current.media&&!directoryRef.current&&!isNativeClosing()&&Boolean(root.current?.isConnected)&&!root.current?.closest('[hidden], [inert]')&&documentSession.current;};
  const adapter:PluginHostAdapter={
   getSnapshot(){const session=available();return session?session.getSnapshot():null;},
   async perform(operation,signal){
    const session=available();if(!session)throw new PluginOperationError('STALE');
    const admit=()=>{
     if(signal.aborted)throw new PluginOperationError('CANCELLED');
     if(available()!==session)throw new PluginOperationError('STALE');
     if(busyRef.current||applyBusyRef.current||applyHeldRef.current||managementHeldRef.current||pluginDocumentState.current.dialog||document.querySelector('dialog[open]'))throw new PluginOperationError('DENIED');
     if(operation.capability==='document.edit'&&!pluginDocumentState.current.editable)throw new PluginOperationError('DENIED');
    };
    try{
     admit();
     if(operation.capability==='document.read')return await session.read(operation.expected,signal,admit);
     if(operation.capability!=='document.edit')throw new PluginOperationError('CAPABILITY_UNSUPPORTED');
     if(!operation.payload)throw new PluginOperationError('INVALID_REQUEST');
     return await session.transact(operation.expected,operation.payload,signal,admit);
    }catch(reason){
     if(reason instanceof RepositoryDocumentSessionError){const codes:Record<typeof reason.code,PluginErrorCode>={cancelled:'CANCELLED',stale:'STALE',denied:'DENIED','read-only':'READ_ONLY',conflict:'CONFLICT',composing:'COMPOSING',invalid:'INVALID_REQUEST',unavailable:'STALE'};throw new PluginOperationError(codes[reason.code]);}
     throw reason;
    }
   },
  };
  return pluginHost.registerDocument(adapter);
 },[pluginHost]);
 useLayoutEffect(()=>{pluginHost?.documentChanged();},[pluginHost,source,path,ready,loading,editing,actualPreview,media,directoryView,active.current.id,active.current.sourceHash,active.current.readOnly,active.current.conflict]);
 ...
 const appliedRepositoryUpdate=async(result:RepositoryUpdateApplied)=>{
  try{
   ...
   pluginHostRef.current?.documentChanged();documentSession.current?.cancelOperations();sessions.current.clear();trashedSessions.current.clear();changedPaths.current.clear();operationPaths.current.clear();savedMessages.current.clear();suspendedCommit.current=null;
   ...
   if(view.current)attachSession(next,view.current);
 ...
 {getNativeBridge()?.resolveReadingReference&&<div className="rfe-reading-tools"><button disabled={busy||loading||historyMoving||applyBusy||applyHeld||managementHeld} onClick={()=>setReferenceOpen(true)}>Open reference…</button>{!directoryView&&evidence?.status==='declared'&&<button disabled={busy||loading||hasFileChanges(active.current)} title={hasFileChanges(active.current)?'Save the draft before copying a saved-content reference':'Copy a portable reference to these saved bytes'} onClick={()=>void copyReadingReference()}>Copy reference</button>}</div>}
 {!directoryView&&evidence&&<ReadingEvidenceContext context={evidence}/>}
 ...
 <section className="rfe-editor-frame" hidden={Boolean(directoryView)}><div className="rfe-toolbar"><div className="rfe-file-toolbar-left"><div className="rfe-modes" role="tablist" aria-label={editing?'Editor mode':'File view'} onKeyDown={event=>{if(!['ArrowLeft','ArrowRight','Home','End'].includes(event.key))return;event.preventDefault();const index=modes.indexOf(selectedMode),next=event.key==='Home'?modes[0]:event.key==='End'?modes.at(-1)!:modes[(index+(event.key==='ArrowRight'?1:-1)+modes.length)%modes.length];changeMode(next);event.currentTarget.querySelector<HTMLButtonElement>(`[data-mode="${next}"]`)?.focus();}}>{modes.map(mode=><button key={mode} id={`${id}-${mode}-tab`} data-mode={mode} role="tab" aria-controls={mode==='split'?`${id}-source-panel ${id}-preview-panel`:`${id}-${mode}-panel`} tabIndex={selectedMode===mode?0:-1} aria-selected={selectedMode===mode} onClick={()=>changeMode(mode)}>{mode==='preview'?'Preview':mode==='split'?'Split':editing?'Edit':'Code'}</button>)}</div>{!editing&&<span className="rfe-file-metadata">{media?'Local media · read only':fileMetadata}</span>}</div>{editing?(actualPreview?<button disabled data-unavailable className="rfe-show-diff">Show diff</button>:<div className="rfe-settings"><select aria-label="Indentation mode" value={space} onChange={event=>setSpace(event.target.value)}><option value="spaces">Spaces</option><option value="tabs">Tabs</option></select><select aria-label="Indentation size" value={size} onChange={event=>setSize(Number(event.target.value))}>{[2,4,8].map(value=><option key={value}>{value}</option>)}</select><select aria-label="Line wrapping" value={wrap?'soft':'off'} onChange={event=>setWrap(event.target.value==='soft')}><option value="soft">Soft wrap</option><option value="off">No wrap</option></select></div>):<div className="rfe-file-actions"><FileSpaceActions/><button className="rfe-icon" aria-label="Copy raw file" title="Copy raw file" disabled={media} onClick={()=>void copyText(active.current.buffer.getRawText(),'Raw file')}><Icon name="copy"/></button><span className="rfe-action-group"><button className="rfe-icon" aria-label="Edit this file" title="Edit this file" disabled={!canEdit} onClick={enterEdit}><Icon name="pencil"/></button>{canEdit?<FileEditOptions onEdit={enterEdit}/>:<button disabled className="rfe-icon" aria-label="More editing options"><span className="rfe-triangle"/></button>}</span></div>}</div>
 <PluginDocumentTools editable={editing&&!actualPreview} disabled={busy||loading||applyBusy||applyHeld||managementHeld||!ready||media||Boolean(directoryView)}/>
 {proEnabled&&editing&&isMarkdownFile(path)&&<ProEditorControls visual={visual} onVisualChange={next=>{if(documentSession.current?.isComposing){setError('Finish text composition before changing the editor view.');return;}setVisual(next);}} disabled={splitActive||busy||loading||!ready}/>}
 {proEnabled&&!editing&&!directoryView&&local&&(/\.html$/i.test(path)||path.endsWith('.artifact.json'))&&<ArtifactViewer key={`${repository}:${path}`} repository={repository} path={path} disabled={busy||loading||!ready||applyBusy||applyHeld||managementHeld}/>}
 <div className="rfe-document-panes" data-split={splitActive||undefined}><div className="rfe-source-pane" hidden={actualPreview||Boolean(directoryView)}>{splitActive&&<div className="rfe-pane-label">Markdown source</div>}<div id={`${id}-source-panel`} role="tabpanel" aria-labelledby={`${id}-${splitActive?'split':'source'}-tab`} ref={mount} className="rfe-source" hidden={actualPreview||Boolean(directoryView)}/></div>{(actualPreview||splitActive)&&<div className="rfe-preview-pane">{splitActive&&<div className="rfe-pane-label">Live draft preview · Synchronized scrolling</div>}<div id={`${id}-preview-panel`} ref={reading} className="rfe-preview rc-document" role="tabpanel" aria-labelledby={`${id}-${splitActive?'split':'preview'}-tab`}>{media?<RepositoryMedia key={JSON.stringify([repository,revision,path])} repository={repository} revision={revision} path={path}/>:rendered?.limited?<p className="rfe-error">This file is too large for the rendered preview. Its source remains available in Edit.</p>:<RepositoryDocument repository={repository} revision={revision} sourcePath={path} fragment={fragment} rendered={rendered??{html:'',images:[]}} onNavigate={(next,hash)=>void openPath(next,undefined,hash)}/>}</div></div>}</div>
