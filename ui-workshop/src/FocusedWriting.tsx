...
import type {ClonedRepository,NativeBuildConfiguration,ResolvedReadingReference} from './native-types';
...
import {NativePluginProvider, NativePluginManager, NativePluginRail, PluginsIcon, usePluginHost} from './NativePluginHost';
...
/** Presentation slots for isolated Storybook studies; not a native plugin API. */
export type WorkspaceStudy = {
  active: boolean;
  rail: React.ReactNode;
  content: React.ReactNode;
  headerContext?: React.ReactNode;
  headerContextReplacesOwner?: boolean;
  onExit(): void;
  sidebar?: {open: boolean; onToggle(): void};
  outline?: {open: boolean; onToggle(): void};
};
type FocusedWritingProps = {pluginFoundation?:boolean;workspaceStudy?:WorkspaceStudy;buildChannel?:NativeBuildConfiguration['channel'];initialSidebar?:boolean;repositoryHeader?:boolean;repositoryCode?:boolean;initialRepository?:string;initialEdit?:boolean;initialDirectory?:string;initialTheme?:string|null;initialHideUnavailable?:boolean;initialSettingsOpen?:boolean;accountPreview?:boolean;initialWorkspaceView?:'home'|'organization'|'repositories'|null};
export function FocusedWriting(props:FocusedWritingProps) {
  const content=<CommitPreferencesProvider enabled={Boolean(props.repositoryHeader)} initialSettingsOpen={props.initialSettingsOpen}><FocusedWritingWindow {...props}/></CommitPreferencesProvider>;
  return getNativeBridge()||props.pluginFoundation?<NativePluginProvider>{content}</NativePluginProvider>:content;
}
function FocusedWritingWindow({workspaceStudy,buildChannel='development',initialSidebar=false,repositoryHeader=false,repositoryCode=false,initialRepository='Workspace',initialEdit=false,initialDirectory,initialTheme=null,initialHideUnavailable=false,accountPreview=false,initialWorkspaceView=null}:FocusedWritingProps) {
  ...
  const openPackages=()=>{void completeNavigation(()=>setPackagesOpen(true));};
  ...
  const [historyReturn,setHistoryReturn]=useState<ReadingHistoryTicket|undefined>(),[referenceReturn,setReferenceReturn]=useState<ResolvedReadingReference|undefined>();
  const readingHistory=useRef<ReadingHistoryAdapter|undefined>(undefined);
  if(!readingHistory.current&&getNativeBridge()?.readingHistory){const bridge=getNativeBridge()!;const call=(input:Parameters<typeof bridge.readingHistory>[0])=>nativeOperation(()=>bridge.readingHistory(input));readingHistory.current={state:()=>call({operation:'state'}),visit:location=>call({operation:'visit',location}),checkpoint:location=>call({operation:'checkpoint',location}),peek:direction=>call({operation:'peek',direction}),complete:(token,location)=>call({operation:'complete',token,location}),cancel:token=>call({operation:'cancel',token})};}
  const loadEvidenceContext=useCallback((input:{repo:string;path:string;ref:string})=>nativeOperation(()=>getNativeBridge()!.getReadingEvidence(input)),[]);
 ...
 const pluginsButton=useRef<HTMLButtonElement>(null);
 const closePlugins=()=>{plugins?.cancelOperations();setPluginsOpen(false);requestAnimationFrame(()=>pluginsButton.current?.focus());};
 ...
 const study:WorkspaceStudy|undefined=pluginsOpen?{active:true,rail:null,content:<NativePluginManager onReturn={closePlugins}/>,headerContext:<strong>Plugins</strong>,onExit:closePlugins}:nativeWindow.native?undefined:workspaceStudy;
 ...
 const completeNavigation=async(action:()=>void)=>{if(isNativeClosing())return;try{setNavigationError('');await beforeLeave.current?.();if(isNativeClosing())return;action();setPendingNavigation(null);}catch(reason){setNavigationError(`Could not preserve the draft: ${(reason as Error).message}`);}};
  ...
  const activateRepository=(name:string,files=false)=>{setHistoryReturn(undefined);setReferenceReturn(undefined);study?.onExit();setNavigationRequest(null);setOutlineState(null);const leavingHome=Boolean(workspaceView);setWorkspaceView(null);setRepository(name);setFilesRequested(files);setRenamedLocation(null);setShowDraft(false);setCreateRepositoryFile(false);setCodeEpoch(value=>value+1);beforeLeave.current=null;editorDirty.current=false;if(leavingHome)focusRepositoryTrigger();};
  const selectRepository=(name:string)=>{if(isNativeClosing()||(name===repository&&!workspaceView&&!study?.active))return;leaveEditor(()=>activateRepository(name));};
  ...
  const navigateHistory=async(ticket:ReadingHistoryTicket)=>{
    const bridge=getNativeBridge();if(!bridge||isNativeClosing())throw Error('Navigation is unavailable while closing.');
    const fresh=await nativeOperation(()=>bridge.catalog());const entry=fresh.repositories.find(value=>value.stableId===ticket.location.repoId);
    if(!entry)throw Error('This repository is no longer available.');
    await nativeOperation(()=>bridge.read({repo:entry.name,path:ticket.location.path,ref:ticket.location.ref}));
    await beforeLeave.current?.();if(isNativeClosing())throw Error('The application is closing.');
    setCatalog(fresh);activateRepository(entry.name,true);setHistoryReturn({...ticket,location:{...ticket.location,repo:entry.name}});
  };
  const navigateReference=async(value:ResolvedReadingReference)=>{
    const bridge=getNativeBridge();if(!bridge||value.status!=='resolved'||!value.repo||!value.path)throw Error('The reference is unavailable.');
    const fresh=await nativeOperation(()=>bridge.catalog()),entry=fresh.repositories.find(item=>item.stableId===value.repoId);
    assertResolvedRepository(value,entry?.stableId);if(!entry)throw Error('The resolved repository is no longer available.');
    await nativeOperation(()=>bridge.read({repo:entry.name,path:value.path!,ref:value.revision??''}));
    await beforeLeave.current?.();if(isNativeClosing())throw Error('The application is closing.');
    setCatalog(fresh);activateRepository(entry.name,true);setReferenceReturn({...value,repo:entry.name});
  };
  ...
  const openWorkspaceView=(view:'home'|'organization'|'repositories')=>{if((workspaceView===view&&!study?.active)||isNativeClosing())return;leaveEditor(()=>{study?.onExit();setWorkspaceView(view);setCatalogEpoch(value=>value+1);},true);};
  ...
  useLayoutEffect(()=>{plugins?.documentChanged();},[plugins,pluginsOpen,workspaceView,windowState]);
  ...
  return <>{windowRest}<div ref={windowRoot} inert={nativeWindow.closing} hidden={windowState!=='open'} className={`fw-window ${nativeWindow.native?'fw-native':''} ${nativeWindow.nativeWindowControls?'fw-native-macos':''} ${expanded?'':'fw-window-restored'} ${hideUnavailable?'fw-hide-unavailable':''}`} style={{...themeStyle,...(windowState!=='open'?{display:'none'}:{})}} onKeyDown={event=>{if(event.key==='Escape'){setMenu(false);if(themeOpen){setThemeOpen(false);themeButton.current?.focus();}}}}>
    ...
    <header className="fw-titlebar">
      {nativeWindow.nativeWindowControls?<div className="fw-native-controls-space" aria-hidden="true"/>:<div className="fw-traffic" role="group" aria-label={nativeWindow.native?'Window controls':'Preview window controls'}>
    ...
    <div className="fw-body">
      <nav className="fw-rail" aria-label="Workspace tools">
        {plugins&&<><button ref={pluginsButton} className="fw-icon pws-rail-button" title="Manage plugins" aria-label="Manage plugins" aria-current={pluginsOpen?'page':undefined} onClick={()=>pluginsOpen?closePlugins():void openPlugins()}><PluginsIcon/></button><NativePluginRail disabled={pluginsOpen||Boolean(workspaceView)}/></>}
        {study?.rail}
        <div className="fw-spacer"/>
        {repositoryHeader?<AccountControls onAutomation={getNativeBridge()?.configureAutomation?()=>setAutomationOpen(true):undefined} preferences={accountPreferences.preferences} loading={accountPreferences.loading} error={accountPreferences.error} onReload={()=>{void accountPreferences.refresh().catch(()=>{});}} onSave={accountPreferences.save} settingsRequest={accountPreferences.settingsRequest} onSettingsClose={accountPreferences.closeSettings} onAppearance={()=>setThemeOpen(true)} application={nativeWindow.native?{
