...
const fail=code=>{throw Object.assign(new Error(code),{code});};const id=persistentIdentity;
...
try{
 await runStorageWorkerEnvelope(JSON.parse(fs.readFileSync(0,'utf8')),r=>{if(id(fs.statSync('.'))!==r.identity)fail('DESTINATION_CHANGED');const parent=fs.openSync('.',fs.constants.O_RDONLY|fs.constants.O_DIRECTORY|flags);let value=null;
