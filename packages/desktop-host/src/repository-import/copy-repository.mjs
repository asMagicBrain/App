...
export async function copyLocalRepository({sourceRoot,destination}){
 ...
 const result=await new Promise((resolve,reject)=>{
  ...
  child.stdout.on('data',bytes=>{output+=bytes.toString('utf8');if(output.length>16384){overflow=true;child.kill('SIGTERM');}});child.stderr.resume();child.stdin.on('error',()=>{});child.stdin.end(JSON.stringify(input));
