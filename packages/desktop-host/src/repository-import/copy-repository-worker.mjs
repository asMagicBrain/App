...
const run=async(destination,identity,request,fds=[])=>{
 ...
 return await new Promise((resolve,reject)=>{
  ...
  child.stdout.on('data',b=>{output+=b.toString('utf8');if(output.length>16384){failed=true;child.kill('SIGTERM');}});child.stderr.resume();child.stdin.on('error',()=>{});child.stdin.end(JSON.stringify({...request,identity}));child.once('error',reject);
...
try{
 const request=JSON.parse(fs.readFileSync(0,'utf8'));if(!leaf(request.source)||!/^\.asmb-import-[a-f0-9-]{36}$/.test(request.destination)||directory('.')!==request.parentIdentity||directory(request.source)!==request.sourceIdentity||directory(request.destination)!==request.targetIdentity)fail('INVALID_ROOT');
