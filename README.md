# asMagicBrain

A desktop workspace for reading and writing Markdown, organizing local files, and keeping a local Git history. Your local work needs no account.

## Get started

1. [Download a preview](https://github.com/asMagicBrain/App/releases).
2. Follow the [installation guide](docs/getting-started.md).
3. Open Workspace and create or import a document.

Preview packages are available for Apple silicon Macs and Ubuntu 24.04 x64. Check [platform requirements and limits](docs/platform-and-limits.md) before installing. The Mac preview is not notarized by Apple.

## Work locally

- Read Markdown with an outline, or switch to Code and Edit.
- Keep unsaved drafts while navigating and after a normal restart.
- Organize files and repositories, import ZIP archives, and search saved content.
- Review saved changes and commit only the files you select.
- Clone from GitHub, compare updates, and apply reviewed updates to a clean repository. Accounts are optional; push and background synchronization are unavailable.

The app includes **asMagicBrain-Docs**, a read-only user guide at the bottom of the repository list. Duplicate it to make an editable copy.

## Learn more

[User guide](docs/README.md) · [Build from source](contributing/development.md) · [Contribute](CONTRIBUTING.md) · [Get help](SUPPORT.md) · [Report a security issue](SECURITY.md)

## Credits and license

Created by **SONG Chaoyang** ([AncoraSIR@gmail.com](mailto:AncoraSIR@gmail.com)) at the [Design and Learning Research Group](https://AncoraSIR.com), with asMagicBrain contributors. Original code and documentation use the [MIT license](LICENSE). Dependencies retain their [own licenses and notices](NOTICE).
